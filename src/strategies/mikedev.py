...
class MikedevStrategy(BaseStrategy):
    ...
    def get_guess(self) -> str:
        ...
        self.round += 1
        guessed_word = max(self.all_words if len(self.words) > 1 else self.words, key=lambda w: self.words_entropy[w])
    ...
    def _update_entropy_(self):
        self.words_entropy = compute_words_entropy_by_letters(self.all_words, self.words)
